...
import com.shengliedu.teacher.teacher.activity.SelectGradeSubjectClassroomtypeTeacher3Activity;
 ...
 * 
 */
public class WeekPlanRight3Adapter extends BaseAdapter {
	private LayoutInflater layoutInflater;
	private List<ClassRoomType> list;
	private Activity context;
	public WeekPlanRight3Adapter(Activity context, List<ClassRoomType> list) {
...
//		viewHolder.week_right_item_grid.setAdapter(adapter);
		viewHolder.week_right_item_grid.setVisibility(View.GONE);
		viewHolder.week_right_item_text.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View arg0) {
				// TODO Auto-generated method stub
				Intent intent = new Intent();
				intent.putExtra("userId", list.get(position).teachers.get(0).id);
				intent.putExtra("gradeId", SelectGradeSubjectClassroomtypeTeacher3Activity.gradeId);
				intent.putExtra("subjectId", SelectGradeSubjectClassroomtypeTeacher3Activity.subjectId);
