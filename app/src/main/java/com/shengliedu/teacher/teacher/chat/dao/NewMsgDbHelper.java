package com.shengliedu.teacher.teacher.chat.dao;
...
public class NewMsgDbHelper {             //新消息条数
	...
	private class SqlLiteHelper extends SQLiteOpenHelper {
		...
		private static final String DB_NAME = "tnewMsg";
		...
		public void onCreate(SQLiteDatabase db) {   //MsgId对聊天来说即对方id
			String sql = "CREATE TABLE  IF NOT EXISTS tnewMsg"
		...
		private void dropTable(SQLiteDatabase db) {
			String sql = "DROP TABLE IF EXISTS tnewMsg";
			db.execSQL(sql);
		}
	...
	public int getMsgCount(String msgId){   
		int count = 0 ;
		String sql ="select msgCount from tnewMsg where msgId=? and whosMsg=?";
	...
	public int getMsgCount(){   
		int count = 0 ;
		String sql ="select sum(msgCount) from tnewMsg where whosMsg=? and msgId != 0";
