...
import com.shengliedu.teacher.teacher.chat.adapter.ChatAdapter;
...
import com.shengliedu.teacher.teacher.chat.view.RecordButton;
import com.shengliedu.teacher.teacher.chat.view.RecordButton.OnFinishedRecordListener;
import com.shengliedu.teacher.teacher.chat.view.expression.ExpressionListener;
import com.shengliedu.teacher.teacher.chat.view.expression.ExpressionView;
...
public class ChatActivity extends D3Activity {
	...
	ImageView leftBtn, rightBtn, sendBtn, picBtn, expBtn, moreBtn;
	@D3View(click = "onClick")
	Button takePicBtn, chosePicBtn, fileBtn, adrBtn;
	@D3View
	...
	protected void onCreate(Bundle arg0) {
		...
		if (!TextUtils.isEmpty(chatName)) {
			VCard vCard = XmppConnection.getInstance().getUserInfo(chatName);
			if (vCard != null) {
		...
		mLocationClient.registerLocationListener(new BDLocationListener() {
			...
			public void onReceiveLocation(BDLocation location) {
				...
				String adr = "[/a0," + location.getLatitude() + ","
	...
	private void initView() {
		adapter = new ChatAdapter(this, chatName);
		...
		msgText.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before,
				...
				if ("".equals(msgText.getText().toString())) {
					sendBtn.setImageResource(R.mipmap.icon_voice);
				} else
					sendBtn.setImageResource(R.mipmap.icon_send_w);
			}
		...
		recordBtn.setOnFinishedRecordListener(new OnFinishedRecordListener() {
			...
			public void onFinishedRecord(String audioPath, int time) {
				if (audioPath != null) {
					try {
						XmppConnection.getInstance().sendMsgWithParms(
								FileUtil.getFileName(audioPath),
								new String[] { "imgData" },
								new Object[] { ImageUtil
										.getBase64StringFromFile(audioPath) },
								chatType);
					} catch (Exception e) {
	...
	public void onClick(View v) {
		...
		case R.id.sendBtn:
			...
			if (!msg.isEmpty()) { // 文本不为空，直接发文本消息
				...
				sendBtn.setImageResource(R.mipmap.icon_keyboard);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (RESULT_OK == resultCode) {
			...
			case PicSrcPickerActivity.CROP:
				...
				if (imgName != null) {
					try {
						XmppConnection.getInstance().sendMsgWithParms(imgName,
								new String[] { "imgData" },
								new Object[] { base64String }, chatType);
					} catch (Exception e) {
						autoSendIfFail(imgName, new String[] { "imgData" },
								new Object[] { base64String });
						e.printStackTrace();
					}
				}
				break;
		...
		timer.schedule(new TimerTask() { // 1秒后开始
					...
					public void run() {
						try {
							count++;
							if (!isLeaving) {
								XmppConnection.getInstance().setRecevier(
										chatName, chatType);
